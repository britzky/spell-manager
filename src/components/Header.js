...
export const Header = () => {
  ...
  return (
    <header>
      <nav className="bg-white border-b-2 border-gray-200 dark:border-b-1 dark:border-gray-900 dark:bg-gray-900">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <Link to="/" className="flex items-center">
              <img src={Logo} className="h-8 mr-3 rounded-md" alt="Cinema Center logo" />
              <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Britzky's Spells</span>
          </Link>
          ...
          <div id="nav-links" className={`${hidden ? "hidden" : "" } items-center justify-between w-full md:flex md:w-auto md:order-1`}> 
            ...
            <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
              ...
              <li>
                <NavLink to={`/filter/${selectedClass}`} className={({isActive}) => isActive ? activeClass : inActiveClass }>Class</NavLink>
              </li>
              <li>
                <NavLink to={`/filter/${selectedSchool}`} className={({isActive}) => isActive ? activeClass : inActiveClass }>School</NavLink>
              </li>
            </ul>
          </div>
        </div>
